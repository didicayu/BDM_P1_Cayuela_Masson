...
\begin{enumerate}
  ...
  \item Run P2: trigger \code{cybersecintel_trusted_zone}, then \code{cybersecintel_warm_stream_aggregates}, then \code{cybersecintel_exploitation_zone}, then \code{cybersecintel_consumption_exports}. The consumption DAG also publishes the Grafana Postgres serving tables.
\end{enumerate}

The P2 stages can also be executed locally after P1 Delta tables exist:
\begin{verbatim}
python -m trusted.run_trusted
python -m ingestion.stream.warm_aggregates
python -m exploitation.run_exploitation --warm --spark-warm
python -m consumption.run_exports
python -m consumption.grafana_postgres
\end{verbatim}
...
C7 Orchestration/containerization & P1 and P2 DAGs run in the Docker Compose Airflow stack; MinIO/Kafka/Postgres/Grafana are containerized.\\
...
\begin{itemize}
  \item Unit test suite covers the Python fallback path, Isolation Forest backend, warm aggregates, Grafana provisioning, Spark job configuration, and governance catalog.
  \item Docker Compose configuration includes MinIO, Kafka, Postgres, Airflow, and Grafana.
...
P2_ENGINE=spark python -m exploitation.run_exploitation --warm --spark-warm
