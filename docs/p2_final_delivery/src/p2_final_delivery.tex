...
\tikzset{
  ...
  capr/.style={-{Latex[length=2.2mm]},draw=csiTeal,semithick,dashed},
}
...
\end{center}
The final P2 implementation is on branch \code{main}. A supervisor can clone and run it locally:
\begin{verbatim}
...
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[node distance=0.5cm and 0.9cm]
    \node[data] (src) {trusted/ids\_alerts\\trusted/suricata\_events};
    \node[data, right=of src] (ne) {exploitation/\\network\_events};
    \node[data, right=of ne] (flags) {exploitation/\\anomaly\_flags};
    \node[data, right=of flags] (serve) {Postgres mirror\\\& Grafana};
    \node[proc, below=1.05cm of flags, text width=4.6cm] (gov) {governance\_lineage \,+\, governance\_quality\_metrics\\(run id, task, rows read/written, status)};
    \draw[arr] (src) -- (ne);
    \draw[arr] (ne) -- (flags);
    \draw[arr] (flags) -- (serve);
    \draw[capr] (src) -- (gov);
    \draw[capr] (ne) -- (gov);
    \draw[capr] (flags) -- (gov);
    \draw[capr] (serve) -- (gov);
  \end{tikzpicture}
  \caption{Embedded governance. Every zone transition (solid) also emits lineage and quality metrics (dashed) as a side effect of data movement, so each product is traceable from Trusted inputs through Exploitation to the Grafana serving layer.}
\end{figure}
...
All P2 stages are orchestrated as Airflow DAGs alongside the P1 ingestion DAGs. The parent \code{cybersecintel_end_to_end} DAG sequences the required path, while each zone also remains independently triggerable for debugging and re-runs.

\begin{figure}[H]
  \centering
  \fcolorbox{csiBoxBorder}{white}{\includegraphics[width=0.98\textwidth,height=0.36\textheight,keepaspectratio]{airflow-screencap.png}}
  \caption{Airflow DAG inventory for the full platform: P1 ingestion (\code{api_expansion_ingestion}, \code{dataset_artifact_ingestion}, \code{pcap_replay}) and P2 (\code{warm_stream_aggregates}, \code{trusted_zone}, \code{exploitation_zone}, \code{consumption_exports}) coordinated by the \code{end_to_end} parent DAG, with recent successful runs.}
\end{figure}
